...
export default {
  ...
  methods: {
    ...
    async getMusicInfo(){
      ...
      try {
        const response = await fetch(`https://meting.arlincode.com/api?server==${this.configdata.musicPlayer.server}&type=${this.configdata.musicPlayer.type}&id=${this.configdata.musicPlayer.id}`
        );
        if (!response.ok) {
